      ...
      <div className="space-y-6">
        <div className="rounded-2xl border border-tool-split/20 bg-tool-split/5 p-6 space-y-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Extract specific pages from your PDF</p>
            </div>
          </div>
          <div className="flex items-start gap-2 rounded-xl bg-card border border-border p-3">
            <Info className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />
            <p className="text-xs text-muted-foreground">
              Select a page range to extract from your PDF. Processed entirely in your browser — your files stay private.
            </p>
          </div>
        </div>
        ...
        <div className="grid gap-2 sm:grid-cols-3">
          {[
            ...
            { step: "2", text: "Enter the page range" },
            { step: "3", text: "Download extracted pages" },
          ].map((s) => (
            <div key={s.step} className="flex items-center gap-2 rounded-xl bg-card border border-border p-3">
              <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-tool-split text-xs font-bold text-primary-foreground">{s.step}</span>
        ...
        {totalPages > 0 && (
          <div className="space-y-4">
            ...
            <Button size="lg" onClick={split} disabled={processing || !pageRange} className="w-full rounded-xl">
              {processing ? <><Loader2 className="h-5 w-5 mr-2 animate-spin" />Splitting…</> : <><Scissors className="mr-2 h-5 w-5" />Split PDF</>}
            </Button>

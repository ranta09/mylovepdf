      ...
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 pt-2">
            <div>
              <Input placeholder="Your email *" type="email" value={email} onChange={e => setEmail(e.target.value)} className="rounded-xl" required />
              <p className="mt-1 text-xs text-muted-foreground">Required so we can follow up</p>
            </div>
            <Textarea placeholder="Describe the issue — which tool, what went wrong, etc." value={text} onChange={e => setText(e.target.value)} rows={4} className="rounded-xl resize-none" />
            <div>
              <label className="flex items-center gap-2 cursor-pointer rounded-xl border border-dashed border-border p-3 hover:bg-secondary/50 transition-colors">
                ...
                <span className="text-sm text-muted-foreground">{screenshot ? screenshot.name : "Attach a screenshot (optional)"}</span>
                <input type="file" accept="image/*" className="hidden" onChange={e => setScreenshot(e.target.files?.[0] || null)} />
              </label>
            </div>
            <Button onClick={handleSubmit} disabled={!text.trim() || !email.trim()} className="w-full rounded-xl">Submit Report</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
